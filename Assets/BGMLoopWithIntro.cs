...
// https://sorceryforce.net/en/tips/unity-music-intro-loop
...
/// </remarks>
public class BGMLoopWithIntro : MonoBehaviour
{
  ...
  void Awake()
  {
    ...
    if (_loopAudioSources[1] != null)
    {
      ...
      _loopAudioSources[1].volume = volume;
        ...
        if (_loopSourceCount >= 2)
        {
          ...
          if (_loopAudioSources[0].time > 0)
          {
            ...
            if (_loopAudioSources[0].time >= AudioClipLoop.length - 1)
            {
              ...
              _loopAudioSources[1]
                .PlayScheduled(AudioSettings.dspTime + (AudioClipLoop.length - _loopAudioSources[0].time));
              _nowPlayIndex = 1;
            }
          }
          else
