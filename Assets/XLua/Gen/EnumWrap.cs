...
namespace XLua.CSObjectWrap
{
    ...
    public class miniRAIDCostTypeWrap
    {
		public static void __Register(RealStatePtr L)
        {
		    ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
		    Utils.BeginObjectRegister(typeof(miniRAID.Cost.Type), L, translator, 0, 0, 0, 0);
			Utils.EndObjectRegister(typeof(miniRAID.Cost.Type), L, translator, null, null, null, null, null);
			
			Utils.BeginClassRegister(typeof(miniRAID.Cost.Type), L, null, 4, 0, 0);

            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "AP", miniRAID.Cost.Type.AP);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Mana", miniRAID.Cost.Type.Mana);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "CD", miniRAID.Cost.Type.CD);
            

			Utils.RegisterFunc(L, Utils.CLS_IDX, "__CastFrom", __CastFrom);
            
            Utils.EndClassRegister(typeof(miniRAID.Cost.Type), L, translator);
        }
        ...
        static int __CastFrom(RealStatePtr L)
		{
            ...
            if (lua_type == LuaTypes.LUA_TNUMBER)
            {
                translator.PushminiRAIDCostType(L, (miniRAID.Cost.Type)LuaAPI.xlua_tointeger(L, 1));
            }
			
            else if(lua_type == LuaTypes.LUA_TSTRING)
            {

			    if (LuaAPI.xlua_is_eq_str(L, 1, "AP"))
                {
                    translator.PushminiRAIDCostType(L, miniRAID.Cost.Type.AP);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "Mana"))
                {
                    translator.PushminiRAIDCostType(L, miniRAID.Cost.Type.Mana);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "CD"))
                {
                    translator.PushminiRAIDCostType(L, miniRAID.Cost.Type.CD);
                }
				else
                {
                    return LuaAPI.luaL_error(L, "invalid string for miniRAID.Cost.Type!");
                }
            ...
            {
                return LuaAPI.luaL_error(L, "invalid lua type for miniRAID.Cost.Type! Expect number or string, got + " + lua_type);
            }
