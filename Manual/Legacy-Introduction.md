...
> 本文保留早期教程与截图，供查阅历史设计。代码示例未按当前版本更新：`Scenes/NewScene`、`SpellTarget.targetPos`、`OnCostQuery` 等不应直接作为当前开发接口；文末关于本地化尚未实现的描述也已过时。当前入口见[手册目录](index.md)，新技能与 Buff 示例见[项目结构](Project-Structure.md)。
